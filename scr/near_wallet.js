...
async function get_NextClaim(Account){
    try {
        ...
        let nextClaim = Math.floor(game_state.last_claim / 1000000) + (Math.floor(Math.random() * (141 - 121 + 1)) + 121) * 60 * 1000;
        return nextClaim;
    }
    catch (err){
        log.error(`Account: ${Account.username} ${err}`);
...
async function claim(Account){
    try {
        ...
        let nextClaimMS = (Math.floor(Math.random() * (141 - 121 + 1)) + 121) * 60 * 1000;
